...
router.patch('/update', auth.authenticateToken, checkRole.checkRole, (req,res,next)=> {
    ...
    connection.query(query, [product.name, product.categoryId, product.description, product.price, product.id], (err, results) => {
        if(!err) {
            ...
            return res.status(200).json({message: "Produto atualizado corretamente"})
        } else {
            return res.status(500).json(err);
        }
    })
...
router.delete('/delete/:id', auth.authenticateToken, checkRole.checkRole, (req,res,next) => {
    ...
    connection.query(query,[id], (err, results) => {
        if(!err) {
            ...
            return res.status(200).json({message: "Produto deletado corretamente"})
        }
        else {
            return res.status(500).json(err);
        }
...
router.patch('/updateStatus', auth.authenticateToken, checkRole.checkRole, (req,res, next) => {
    ...
    connection.query(query,[user.status,user.id], (err, results) => {
        if(!err) {
            ...
            return res.status(200).json({message: "Status do Produto atualizado corretamente"})
        }
        else {
            return res.status(500).json(err);
        }
